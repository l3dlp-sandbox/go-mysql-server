...
func TestAssignCatalog(t *testing.T) {
	...
	node, err := f.Apply(ctx, a,
		plan.NewCreateIndex("", plan.NewResolvedTable(tbl, nil, nil), nil, "", make(map[string]string)), nil, SelectAll)
	require.NoError(err)
	...
	node, err = f.Apply(ctx, a,
		plan.NewDropIndex("foo", plan.NewResolvedTable(tbl, nil, nil)), nil, SelectAll)
	require.NoError(err)
	...
	node, err = f.Apply(ctx, a, plan.NewShowProcessList(), nil, SelectAll)
	require.NoError(err)
	...
	node, err = f.Apply(ctx, a, plan.NewShowDatabases(), nil, SelectAll)
	require.NoError(err)
	sd, ok := node.(*plan.ShowDatabases)
	require.True(ok)
	require.Equal(a.Catalog, sd.Catalog)

	node, err = f.Apply(ctx, a, plan.NewLockTables(nil), nil, SelectAll)
	require.NoError(err)
	lt, ok := node.(*plan.LockTables)
	require.True(ok)
	require.Equal(a.Catalog, lt.Catalog)

	node, err = f.Apply(ctx, a, plan.NewUnlockTables(), nil, SelectAll)
	require.NoError(err)
