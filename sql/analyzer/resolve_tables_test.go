...
func TestResolveTables(t *testing.T) {
	...
	analyzed, err := f.Apply(ctx, a, notAnalyzed, nil, SelectAll)
	require.NoError(err)
	...
	analyzed, err = f.Apply(ctx, a, plan.NewResolvedTable(table, db, nil), nil, SelectAll)
	require.NoError(err)
	require.Equal(plan.NewResolvedTable(table, db, nil), analyzed)
...
func TestResolveTablesNoCurrentDB(t *testing.T) {
	...
	var notAnalyzed sql.Node = plan.NewUnresolvedTable("mytable", "")
	_, err := f.Apply(ctx, a, notAnalyzed, nil, SelectAll)
	require.Error(err)
