...
func TestResolveSubqueries(t *testing.T) {
	...
	runTestCases(t, ctx, testCases, a, Rule{
		Name: "subqueries",
		Apply: func(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) (sql.Node, error) {
			n, err := resolveSubqueries.Apply(ctx, a, n, scope, SelectAll)
			if err != nil {
				return nil, err
			}
			return finalizeSubqueries.Apply(ctx, a, n, scope, SelectAll)
		},
	})
}
