...
func TestValidateResolved(t *testing.T) {
	...
	_, err := vr.Apply(sql.NewEmptyContext(), nil, dummyNode{true}, nil, SelectAll)
	require.NoError(err)

	_, err = vr.Apply(sql.NewEmptyContext(), nil, dummyNode{false}, nil, SelectAll)
	require.Error(err)
}
...
func TestValidateGroupBy(t *testing.T) {
	...
	_, err = vr.Apply(sql.NewEmptyContext(), nil, p, nil, SelectAll)
	require.NoError(err)
}
...
func TestValidateSchemaSource(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			require := require.New(t)
			_, err := rule.Apply(sql.NewEmptyContext(), nil, tt.node, nil, SelectAll)
			if tt.ok {
				require.NoError(err)
			} else {
				require.Error(err)
...
func TestValidateIntervalUsage(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := validateIntervalUsage(sql.NewEmptyContext(), nil, tt.node, nil, SelectAll)
			if tt.ok {
				require.NoError(err)
			} else {
				require.Error(err)
...
func TestValidateExplodeUsage(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := validateExplodeUsage(sql.NewEmptyContext(), nil, tt.node, nil, SelectAll)
			if tt.ok {
				require.NoError(err)
			} else {
				require.Error(err)
...
func TestValidateSubqueryColumns(t *testing.T) {
	...
	_, err := validateSubqueryColumns(ctx, nil, node, nil, SelectAll)
	require.NoError(err)
