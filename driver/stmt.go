...
func (s *Stmt) exec(ctx context.Context, bindings map[string]sql.Expression) (driver.Result, error) {
	...
	_, rows, err := s.conn.dbConn.engine.QueryWithBindings(qctx, 0, s.queryStr, bindings)
	if err != nil {
		return nil, err
	}
...
func (s *Stmt) query(ctx context.Context, bindings map[string]sql.Expression) (driver.Rows, error) {
	...
	cols, rows, err := s.conn.dbConn.engine.QueryWithBindings(qctx, 0, s.queryStr, bindings)
	if err != nil {
		return nil, err
	}
